# Hi 👋, I'm Purini Dharmendra

💻 Aspiring Software Developer | Python & Django Enthusiast  
📍 Bangalore, India  

---

## 🚀 About Me

I am a highly motivated Computer Science graduate (2025) from **Visvodaya Engineering College**.  
I enjoy building web applications and learning new technologies.

I completed a **Python Full Stack Development Internship at ExcelR**, where I gained hands-on experience building dynamic web applications using Django and web technologies.

I am currently looking for **entry-level opportunities in Software Development / QA / Automation Testing** where I can apply my skills and grow in the IT industry.

---

## 🛠️ Skills

- Python
- SQL
- HTML
- CSS
- JavaScript
- Django
- Excel

---

## 💼 Internship

### Python Full Stack Development Intern – ExcelR  
📅 July 2024 – August 2024  

- Worked on web development using **Python and Django**
- Built dynamic web pages using **HTML, CSS, JavaScript**
- Learned database operations using **SQL**
- Improved problem-solving and debugging skills

---

## 📊 Project

### Improving Shopping Mall Revenue using Real-Time Digital Coupons

Developed a concept-based project that uses **real-time data analytics** to provide **personalized digital coupons to customers inside shopping malls**.

Key idea:
- Track customer behavior
- Send targeted offers to smartphones
- Increase sales and customer engagement

Technologies used:
- Python
- Django
- SQL
- Web Technologies

---

## 🎓 Education

**B.Tech in Computer Science Engineering**  
Visvodaya Engineering College, Nellore  
📅 Graduated: 2025  
CGPA: **6.98**

---

## 📫 Contact Me

📧 Email: purinidharma@gmail.com  
📱 Phone: +91 9640581310  

---

⭐ *Always eager to learn new technologies and contribute to meaningful projects.*
